...
@router.get("/users/current/{username}", dependencies=[Depends(seguranca.get_current_user)], response_model=entidades.User)
...
async def get_all_users(user: entidades.User = Depends(seguranca.get_current_user), db: Session = Depends(banco.get_db)):
  # TODO: Check user permissions (replace with your access control logic)
  ...
  filtered_users = [
      entidades.User(id=u.id, username=u.username, email=u.email, password="?")
      for u in banco.get_all_users(db)
  ]

  # TODO Optionally, filter further based on logged-in user
  if user.id:
...
@router.get("/users/{user_id}", dependencies=[Depends(seguranca.get_current_user)], response_model=entidades.User)
...
async def get_user_role(user: entidades.User, db: Session = Depends(banco.get_db)) -> str:
    # TODO: Implement logic to get user role from database based on user.id
    # ...
...
async def has_access(user: entidades.User = Depends(seguranca.get_current_user), permission: str = "list_users") -> bool:
    ...
    allowed_roles = {"list_users": ["admin"]}  # TODO: Map permission to allowed roles
